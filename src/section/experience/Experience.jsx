...
const Experience = () => {
  ...
  return (
    <div className="experience w-full   ">
      <p className="capitalize p-5 w-80 text-[25px] font-bold text-white hover-target">
        My work Experience
      </p>
      <div className="grid xl:grid-cols-5 md:grid-cols-1 max-sm:grid-cols-1  ">
        <div className=" xl:col-span-2  lg:border-r  border-b max-sm:h-screen ">
          <Canvas>
            <ambientLight intensity={7} />
            <spotLight position={[10, 7, 5]} angle={0.15} penumbra={1} />
            <directionalLight position={[10, 7, 5]} intensity={1} />
        ...
        <div className=" xl:col-span-3 h-auto items-center p-3">
            ...
            {experiencecontant.map((item) => {
              return (
                <div className="px-3 hover:border hover:border-gray-300 transition-transform duration-200 ease-in-out rounded-md">
                  ...
                  <p className="ps-12 py-2 hover-target">{item?.title}</p>
                </div>
              );
            })}
          </div>
